...
public sealed class ArtifactHeatTriggerSystem : EntitySystem
{
    ...
    private void OnAttacked(Entity<ArtifactHeatTriggerComponent> ent, ref AttackedEvent args)
    {
        if (!ent.Comp.ActivateHotItems || !CheckHot(args.Used))
            return;
        _artifactSystem.TryActivateArtifact(ent, args.User);
    }

    private void OnUsing(Entity<ArtifactHeatTriggerComponent> ent, ref InteractUsingEvent args)
    {
        if (args.Handled)
            return;
