...
public sealed partial class OldAnalysisConsoleMenu : FancyWindow
{
    [Dependency] private readonly IEntityManager _ent = default!;
    [Dependency] private readonly IGameTiming _timing = default!;

    public event Action? OnOldServerSelectionButtonPressed;
    public event Action? OnScanButtonPressed;
    public event Action? OnPrintButtonPressed;
    public event Action? OnOldExtractButtonPressed;
    ...
    public OldAnalysisConsoleMenu()
    {
        RobustXamlLoader.Load(this);
        IoCManager.InjectDependencies(this);

        OldServerSelectionButton.OnPressed += _ => OnOldServerSelectionButtonPressed?.Invoke();
        ScanButton.OnPressed += _ => OnScanButtonPressed?.Invoke();
        PrintButton.OnPressed += _ => OnPrintButtonPressed?.Invoke();
        OldExtractButton.OnPressed += _ => OnOldExtractButtonPressed?.Invoke();
    ...
    protected override void FrameUpdate(FrameEventArgs args)
    {
        ...
        ProgressLabel.Text = Loc.GetString("old-analysis-console-progress-text",
            ("seconds", secsText));
    ...
    public void SetButtonsDisabled(OldAnalysisConsoleUpdateState state)
    {
        ...
        OldExtractButton.Disabled = false;
        if (!state.ServerConnected)
        {
            OldExtractButton.Disabled = true;
            OldExtractButton.ToolTip = Loc.GetString("old-analysis-console-no-server-connected");
        }
        else if (!state.CanScan)
        {
            ...
            {
                OldExtractButton.ToolTip = Loc.GetString("old-analysis-console-no-artifact-placed");
            }
            else
            {
                OldExtractButton.ToolTip = null;
            }
        }
        else if (state.PointAmount <= 0)
        {
            OldExtractButton.Disabled = true;
            OldExtractButton.ToolTip = Loc.GetString("old-analysis-console-no-points-to-extract");
        }

        if (OldExtractButton.Disabled)
        {
            OldExtractButton.RemoveStyleClass("ButtonColorGreen");
        }
        else
        {
            OldExtractButton.AddStyleClass("ButtonColorGreen");
            OldExtractButton.ToolTip = null;
        }
    }
    ...
    public void UpdateInformationDisplay(OldAnalysisConsoleUpdateState state)
    {
        ...
        if (state.Scanning)
        {
            if (state.Paused)
            {
                message.AddMarkupOrThrow(Loc.GetString("old-analysis-console-info-scanner-paused"));
            }
            else
            {
                message.AddMarkupOrThrow(Loc.GetString("old-analysis-console-info-scanner"));
            }
        ...
        if (state.ScanReport == null)
        {
            if (!state.AnalyzerConnected) //no analyzer connected
                message.AddMarkupOrThrow(Loc.GetString("old-analysis-console-info-no-scanner"));
            else if (!state.CanScan) //no artifact
                message.AddMarkupOrThrow(Loc.GetString("old-analysis-console-info-no-artifact"));
            else if (state.Artifact == null) //ready to go
                message.AddMarkupOrThrow(Loc.GetString("old-analysis-console-info-ready"));
        }
        else
        {
    ...
    public void UpdateProgressBar(OldAnalysisConsoleUpdateState state)
    {
